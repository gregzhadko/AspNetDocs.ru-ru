...
title: Учебник. Изучение сложных сценариев EF для веб-приложения MVC 5
...
ms.locfileid: "86188735"
---
# <a name="tutorial-learn-about-advanced-ef-scenarios-for-an-mvc-5-web-app"></a>Учебник. Изучение сложных сценариев EF для веб-приложения MVC 5
...
- Используйте [Database.Exeкутесклкомманд](https://msdn.microsoft.com/library/gg679456.aspx) для команд, не относящихся к запросу.
...
В *DepartmentController.CS*в `Details` методе замените `db.Departments.FindAsync` вызов метода на `db.Departments.SqlQuery` вызов метода, как показано в следующем выделенном коде:
...
Удалите `var sql = courses.ToString()` строку.
...
- [Рик Андерсон (](https://blogs.msdn.com/b/rickandy/) (Twitter [@RickAndMSFT](http://twitter.com/RickAndMSFT) ) выполнил большую часть работы по обновлению руководства по EF 5 и MVC 4 и соавтору обновления EF 6. Рик — старший специалист по программированию для Microsoft, сосредоточенный на Azure и MVC.
...
Сообщение об ошибке (из `Update-Database` команды в PMC):

> Термин "обновление базы данных" не распознается как имя командлета, функции, файла сценария или исполняемой программы. Проверьте правильность написания имени, а если включен путь, то проверьте правильность пути и повторите попытку.
