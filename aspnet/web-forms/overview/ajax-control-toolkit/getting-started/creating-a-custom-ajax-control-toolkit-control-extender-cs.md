...
([Щелкните, чтобы просмотреть изображение с полным размером](creating-a-custom-ajax-control-toolkit-control-extender-cs/_static/image3.png))
...
([Щелкните, чтобы просмотреть изображение с полным размером](creating-a-custom-ajax-control-toolkit-control-extender-cs/_static/image6.png))

Чтобы создать расширитель элемента управления, необходимо создать следующие три файла:

- DisabledButtonExtender.cs — этот файл является классом элемента управления на стороне сервера, который будет управлять созданием расширителя и позволяет задавать свойства во время разработки. Он также определяет свойства, которые можно задать для расширителя. Эти свойства доступны в коде и во время разработки и соответствуют свойствам, определенным в файле DisableButtonBehavior.js.
- DisabledButtonBehavior.js--в этом файле будет добавлена вся логика клиентского скрипта.
- DisabledButtonDesigner.cs — этот класс включает функции времени разработки. Этот класс необходим, если требуется, чтобы расширитель элемента управления правильно работал с конструктором Visual Studio или Visual Web Developer.
...
Первым шагом является создание проекта библиотеки классов и веб-сайта в Visual Studio или Visual Web Developer. Мы создадим пользовательский расширитель в проекте библиотеки классов и протестируем пользовательский расширитель на веб-сайте.
...
[![Решение с веб-сайтом и проектом библиотеки классов](creating-a-custom-ajax-control-toolkit-control-extender-cs/_static/image8.png)](creating-a-custom-ajax-control-toolkit-control-extender-cs/_static/image7.png)

**Рис. 01**. решение с проектом веб-сайта и библиотеки классов ([щелкните, чтобы просмотреть изображение с полным размером](creating-a-custom-ajax-control-toolkit-control-extender-cs/_static/image9.png))
...
4. Откройте вкладку Browse (Обзор).
5. Добавьте ссылку на сборку AjaxControlToolkit.dll. Эта сборка находится в папке, в которую был скачан набор средств AJAX Control Toolkit.

После выполнения этих действий папка ссылок проекта библиотеки классов должна выглядеть, как показано на рис. 2.

[![Папка References с необходимыми ссылками](creating-a-custom-ajax-control-toolkit-control-extender-cs/_static/image11.png)](creating-a-custom-ajax-control-toolkit-control-extender-cs/_static/image10.png)

**Рис. 02**. ссылки на папку с необходимыми ссылками ([щелкните, чтобы просмотреть изображение с полным размером](creating-a-custom-ajax-control-toolkit-control-extender-cs/_static/image12.png))
...
Атрибут «расширение ресурсов» используется для внедрения файла MyControlBehavior.js JavaScript в сборку при компиляции пользовательского расширителя. Атрибут Клиентскриптресаурце используется для получения скрипта MyControlBehavior.js из сборки, когда пользовательский расширитель используется на веб-странице.
...
Давайте выполним код для нашего расширителя Дисабледбуттон. Код для этого расширителя можно найти в листинге 2.
...
Атрибуты WebClient и Клиентскриптресаурце связывают поведение на стороне клиента, расположенное в файле с именем DisabledButtonBehavior.js, с этим расширителем. Этот файл JavaScript обсуждается в следующем разделе.
...
Файл JavaScript в листинге 3 содержит класс на стороне клиента с именем Дисабледбуттонбехавиор. Этот класс, как и двойника на стороне сервера, содержит два свойства с именами Таржетбуттонид и Дисабледтекст, к которым можно получить доступ с помощью Get \_ таржетбуттонид/Set \_ таржетбуттонид и Get \_ дисабледтекст/Set \_ дисабледтекст.
...
**Рис. 03**. Добавление файла JavaScript в качестве внедренного ресурса ([щелкните, чтобы просмотреть изображение с полным размером](creating-a-custom-ajax-control-toolkit-control-extender-cs/_static/image15.png))
...
3. В диалоговом окне Выбор элементов панели элементов перейдите к CustomExtenders.dll сборке.
...
[![Дисабледбуттон на панели элементов](creating-a-custom-ajax-control-toolkit-control-extender-cs/_static/image17.png)](creating-a-custom-ajax-control-toolkit-control-extender-cs/_static/image16.png)

**Рис. 04**. дисабледбуттон на панели элементов ([щелкните, чтобы просмотреть изображение с полным размером](creating-a-custom-ajax-control-toolkit-control-extender-cs/_static/image18.png))
...
5. В окно свойств измените свойство идентификатора кнопки на значение <em>бтнсаве</em> , а свойство Text — на значение *Save \* *.
...
**Рис. 05**. диалоговое окно мастера расширения ([щелкните, чтобы просмотреть изображение с полным размером](creating-a-custom-ajax-control-toolkit-control-extender-cs/_static/image21.png))
...
**Рис. 06**. Задание свойств расширителя ([щелкните, чтобы просмотреть изображение с полным размером](creating-a-custom-ajax-control-toolkit-control-extender-cs/_static/image24.png))
...
[![Расширитель Дисабледбуттон в действии](creating-a-custom-ajax-control-toolkit-control-extender-cs/_static/image26.png)](creating-a-custom-ajax-control-toolkit-control-extender-cs/_static/image25.png)

**Рис. 07**. расширитель дисабледбуттон в действии ([щелкните, чтобы просмотреть изображение с полным размером](creating-a-custom-ajax-control-toolkit-control-extender-cs/_static/image27.png))
