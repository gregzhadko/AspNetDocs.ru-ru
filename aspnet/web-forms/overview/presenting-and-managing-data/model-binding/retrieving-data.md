...
>
> Вы можете [скачать](https://go.microsoft.com/fwlink/?LinkId=286116) полный проект на C# или Visual Basic. Загружаемый код работает с Visual Studio 2012 и более поздними версиями. В нем используется шаблон Visual Studio 2012, который немного отличается от шаблона Visual Studio 2017, показанного в этом руководстве.
> 
...
> [Бесплатная пробная учетная запись Azure](https://azure.microsoft.com/free/dotnet/). Сведения о том, как развернуть веб-проект Visual Studio в веб-приложениях службы приложений Azure, см. в разделе [веб-развертывание ASP.NET с использованием Visual Studio](../../deployment/visual-studio-web-deployment/introduction.md) Series. В этом учебнике также показано, как использовать Entity Framework Code First Migrations для развертывания базы данных SQL Server в базе данных SQL Azure.
> 
...
В папке **Models** добавьте класс с именем **UniversityModels.CS**.
   ...
   3. Назовите класс **UniversityModels.CS** и выберите **Добавить**.
      ...
      `SchoolContext`Класс является производным от `DbContext` , который управляет подключением к базе данных и изменениями в данных.
...
В этом руководстве для создания объектов и таблиц базы данных используется [Code First migrations](https://docs.microsoft.com/ef/ef6/modeling/code-first/migrations/) . В этих таблицах хранятся сведения о учащихся и их курсах.
   ...
   2. В **консоли диспетчера пакетов**выполните следующую команду:  
      ...
      Обратите внимание, что был создан файл с именем *Configuration.CS* . `Configuration`Класс содержит `Seed` метод, который может заранее заполнить таблицы базы данных тестовыми данными.
