...
## <a name="how-requests-are-processed-by-the-thread-pool"></a><a id="HowRequestsProcessedByTP"></a>  Обработка запросов пулом потоков
...
## <a name="choosing-synchronous-or-asynchronous-methods"></a><a id="ChoosingSyncVasync"></a>  Выбор синхронных или асинхронных методов
...
## <a name="the-sample-application"></a><a id="SampleApp"></a>  Пример приложения

Загрузить пример приложения можно [https://github.com/RickAndMSFT/Async-ASP.NET](https://github.com/RickAndMSFT/Async-ASP.NET) на сайте [GitHub](https://github.com/) . Репозиторий состоит из трех проектов:
...
- *Вебапипгв*: проект веб-API ASP.NET MVC 4, который реализует `Products, Gizmos and Widgets` контроллеры. Он предоставляет данные для проекта *вебаппасинк* и проекта *Mvc4Async* .
...
## <a name="the-gizmos-synchronous-page"></a><a id="GizmosSynch"></a>  Синхронная страница приспособлений
...
В следующем коде показан `GetGizmos` метод службы гизмо.

[!code-csharp[Main](using-asynchronous-methods-in-aspnet-45/samples/sample2.cs)]

`GizmoService GetGizmos`Метод передает универсальный код ресурса (URI) веб-API ASP.NET службе HTTP, которая возвращает список данных приспособлений. Проект *вебапипгв* содержит реализацию веб-API `gizmos, widget` и `product` контроллеров.  
...
## <a name="creating-an-asynchronous-gizmos-page"></a><a id="CreatingAsynchGizmos"></a>  Создание асинхронной страницы приспособлений

В примере используются новые ключевые слова [Async](https://msdn.microsoft.com/library/hh156513(VS.110).aspx) и [await](https://msdn.microsoft.com/library/hh156528(VS.110).aspx) (доступные в .NET 4,5 и Visual Studio 2012), позволяющие компилятору отвечать за поддержание сложных преобразований, необходимых для асинхронного программирования. Компилятор позволяет писать код с помощью синхронных конструкций потока управления C#, и компилятор автоматически применяет преобразования, необходимые для использования обратных вызовов, чтобы избежать блокировки потоков.

ASP.NET асинхронные страницы должны содержать директиву [Page](https://msdn.microsoft.com/library/ydy4x04a.aspx) с `Async` атрибутом, имеющим значение true. В следующем коде показана директива [страницы](https://msdn.microsoft.com/library/ydy4x04a.aspx) с `Async` атрибутом, для которого задано значение true для страницы *гизмосасинк. aspx* .

[!code-aspx[Main](using-asynchronous-methods-in-aspnet-45/samples/sample3.aspx?highlight=1)]

В следующем коде показан `Gizmos` синхронный `Page_Load` метод и `GizmosAsync` асинхронная страница. Если браузер поддерживает [ &lt; &gt; элемент HTML 5 Mark](http://www.w3.org/wiki/HTML/Elements/mark), то изменения отображаются в `GizmosAsync` желтом выделе.
...
[!code-csharp[Main](using-asynchronous-methods-in-aspnet-45/samples/sample5.cs?highlight=3,6-7,9,11)]
 ...
 Для обеспечения асинхронности страницы были применены следующие изменения `GizmosAsync` .

- Директива [страницы](https://msdn.microsoft.com/library/ydy4x04a.aspx) должна иметь `Async` атрибут со значением "true".
- `RegisterAsyncTask`Метод используется для регистрации асинхронной задачи, содержащей код, который выполняется асинхронно.
- Новый `GetGizmosSvcAsync` метод помечается ключевым словом [Async](https://msdn.microsoft.com/library/hh156513(VS.110).aspx) , которое указывает компилятору создавать обратные вызовы для частей тела и автоматически создавать `Task` возвращаемый объект.
...
- Тип возвращаемого значения нового `GetGizmosSvcAsync` метода — `Task` . Тип возвращаемого значения `Task` представляет текущую работу и предоставляет вызывающие методы метода с помощью обработчика, который ожидает завершения асинхронной операции.
- Ключевое слово [await](https://msdn.microsoft.com/library/hh156528(VS.110).aspx) было применено к вызову веб-службы.
- API асинхронной веб-службы был вызван ( `GetGizmosAsync` ).

Внутри `GetGizmosSvcAsync` тела метода вызывается другой асинхронный метод `GetGizmosAsync` . `GetGizmosAsync` немедленно возвращает объект `Task<List<Gizmo>>` , который в конечном итоге завершится, когда данные станут доступны. Так как вы не хотите делать что-то еще, пока не гизмо данные, код ожидает задачу (с помощью ключевого слова **await** ). Ключевое слово **await** можно использовать только в методах, снабженных ключевым словом **Async** .
...
Методы, подключенные к, `RegisterAsyncTask` будут выполняться сразу после [PreRender](https://msdn.microsoft.com/library/ms178472.aspx).

При непосредственном использовании асинхронных событий страницы void, как показано в следующем коде:

[!code-csharp[Main](using-asynchronous-methods-in-aspnet-45/samples/sample8.cs)]

у вас больше нет возможности полностью контролировать время выполнения событий. Например, если и. aspx, и. События главного определения `Page_Load` и одно или оба из них являются асинхронными, порядок выполнения не гарантируется. Применяется тот же неопределенный порядок для обработчиков событий (например, `async void Button_Click` ).

## <a name="performing-multiple-operations-in-parallel"></a><a id="Parallel"></a>  Параллельное выполнение нескольких операций

Асинхронные методы имеют значительное преимущество по сравнению с синхронными методами, когда действие должно выполнять несколько независимых операций. В приведенном образце синхронная страница *ПВГ. aspx*(для продуктов, мини-приложений и приспособлений) отображает результаты трех вызовов Web Service, чтобы получить список продуктов, мини-приложений и приспособлений. Проект [веб-API ASP.NET](../../../web-api/index.md) , предоставляющий эти службы, использует [Task. Delay](https://msdn.microsoft.com/library/hh139096(VS.110).aspx) для имитации задержки или медленных сетевых вызовов. Если задержка устанавливается равным 500 миллисекундам, то асинхронная страница *пвгасинк. aspx* занимает немного больше 500 миллисекунд, пока синхронная `PWG` версия занимает свыше 1 500 миллисекунд. В следующем коде показана синхронная страница *ПВГ. aspx* .

[!code-csharp[Main](using-asynchronous-methods-in-aspnet-45/samples/sample9.cs)]

`PWGasync`Ниже показан асинхронный код программной части.
...
## <a name="using-a-cancellation-token"></a><a id="CancelToken"></a>  Использование токена отмены

Асинхронные методы, возвращаемые `Task` , являются отменяемыми, то есть принимают параметр [CancellationToken](https://msdn.microsoft.com/library/system.threading.cancellationtoken(VS.110).aspx) , когда он предоставляется с `AsyncTimeout` атрибутом директивы [Page](https://msdn.microsoft.com/library/ydy4x04a.aspx) . В следующем коде показана страница *гизмосканцеласинк. aspx* с временем ожидания в секундах.
...
## <a name="server-configuration-for-high-concurrencyhigh-latency-web-service-calls"></a><a id="ServerConfig"></a>  Конфигурация сервера для вызовов веб-службы с высоким уровнем параллелизма и высокой задержкой
...
- Возможно, потребуется увеличить предельный размер очереди [HTTP.sys](https://www.iis.net/learn/get-started/introduction-to-iis/introduction-to-iis-architecture) по умолчанию — от 1 000 до 5 000. Если параметр слишком мал, [HTTP.sys](https://www.iis.net/learn/get-started/introduction-to-iis/introduction-to-iis-architecture) отклонять запросы с состоянием HTTP 503. Чтобы изменить ограничение очереди HTTP.sys:
    ...
    - Щелкните правой кнопкой мыши целевой пул приложений и выберите **Дополнительные параметры**.  
        ![продвинут](using-asynchronous-methods-in-aspnet-45/_static/image4.png)
...
- Если приложение использует веб-службы или System.NET для взаимодействия с серверной частью по протоколу HTTP, может потребоваться увеличить элемент [элемент connectionManagement/maxConnection](https://msdn.microsoft.com/library/fb6y0fyc(VS.110).aspx) . Для приложений ASP.NET это ограничено функцией автонастройки в 12 раз больше, чем количество процессоров. Это означает, что на четырехъядерном уровне можно использовать не более 12 \* 4 = 48 одновременных подключений к конечной точке IP. Поскольку это связано с [автоматической конфигурацией](https://msdn.microsoft.com/library/7w2sway1(VS.110).aspx), самый простой способ увеличить `maxconnection` ASP.NET приложение — установить [System .NET. ServicePointManager. DefaultConnectionLimit](https://msdn.microsoft.com/library/system.net.servicepointmanager.defaultconnectionlimit(VS.110).aspx) программно в `Application_Start` метод from в файле *Global. asax* . Пример см. в примере загрузки.
