...
1. Откройте Visual Studio 2013 с правами администратора. Выберите **файл**, **создать проект**. Выберите **Windows** в узле **Visual C#** в области **шаблоны** и выберите шаблон **консольное приложение** . Назовите новый проект "Сигналрселфхост" и нажмите кнопку **ОК**.
...
5. Замените содержимое файла Program.cs на код, приведенный ниже.
    ...
    - **Startup**, класс, содержащий конфигурацию сервера SignalR (единственной конфигурацией, используемой в этом руководстве, является вызов `UseCors` ) и вызов метода `MapSignalR` , который создает маршруты для всех объектов-концентраторов в проекте.
...
7. В случае сбоя выполнения с исключением необходимо `System.Reflection.TargetInvocationException was unhandled` перезапустить Visual Studio с правами администратора.
...
4. Щелкните проект правой кнопкой мыши и выберите **Добавить**, **новый элемент**. Выберите **веб-** узел и выберите HTML-страница. Присвойте странице имя **Default.html**.
